...
VegBank_plot_data_NC_fringe <- read_excel("/home/shares/neon-inv/raw_VegBank_data/CVS-NCFringePlots.xlsx", sheet = "plot data")
VegBank_plot_species_list_NC_fringe  <- read_excel("/home/shares/neon-inv/raw_VegBank_data/CVS-NCFringePlots.xlsx",  sheet = "plot species list")
...
VegBank_NC_fringe <- full_join(VegBank_plot_data_NC_fringe2, VegBank_plot_species_list_NC_fringe2, by = 'UniqueID')
...
VegBank_NC_fringe <- VegBank_NC_fringe %>% add_column(Dataset = 'VEGBANK', .before = "UniqueID")  %>% add_column(Zone = 'L48', .before = "UniqueID")

#Making exotic status compatible to NEON data
VegBank_NC_fringe$ExoticStatus <- gsub("native", "N", VegBank_NC_fringe$ExoticStatus)
VegBank_NC_fringe$ExoticStatus <- gsub("NATIVE", "N", VegBank_NC_fringe$ExoticStatus)
VegBank_NC_fringe$ExoticStatus <- gsub("exotic", "NI", VegBank_NC_fringe$ExoticStatus)
VegBank_NC_fringe$ExoticStatus <- gsub("EXOTIC", "NI", VegBank_NC_fringe$ExoticStatus)
...
write.csv(VegBank_multiple_species_NC_fringe ,"/home/shares/neon-inv/output_files/multiple_sp_NC_fringe_KPEACH.csv", row.names = FALSE)
...
#1,993 rows with no match
no_match <- dplyr::anti_join(VegBank_NC_fringe,USDA_plant_list_ALL, by="bestname")
...
#Save as csv for Jeff to inspect
write.csv(no_match,"/home/shares/neon-inv/output_files/no_match_NC_fringe_KPEACH.csv", row.names = FALSE)
...
#11,467
VegBank_with_SpCode_NC_fringe1 <- dplyr::inner_join(VegBank_NC_fringe, USDA_plant_list_ALL, by="bestname")

VegBank_with_SpCode_NC_fringe2 <- dplyr::inner_join(no_match, USDA_plant_list_ALL, by="bestname")

VegBank_with_SpCode_NC_fringe <- merge(VegBank_with_SpCode_NC_fringe1, VegBank_with_SpCode_NC_fringe2, all = TRUE)
...
#0! Yay!
no_match2 <- dplyr::anti_join(VegBank_with_SpCode_NC_fringe,USDA_plant_list_ALL, by="bestname")
...
plot_sizes <- unique(VegBank_with_SpCode_NC_fringe$`Taxon Observation Area`)

odd_plot_sizes <- VegBank_with_SpCode_NC_fringe %>%
  filter(`Taxon Observation Area` == '-1')

#write.csv(odd_plot_sizes,"odd_plot_sizes_NC_fringe_KPEACH.csv", row.names = FALSE)
...
#As csv
write.csv(VegBank_with_SpCode_NC_fringe,"/home/shares/neon-inv/output_files/VegBank_NC_fringe_KPEACH.csv", row.names = FALSE)
