...
USDA_list_w_exotic_status$Native.Status <- substr(USDA_list_w_exotic_status$Native.Status, 5, 6)
...
add_row(bestname = 'Viola', SpCode = 'VIOLA')

USDA_plant_list_ALL <- USDA_plant_list_ALL %>% 
add_row(bestname = 'Polygonum', SpCode = 'POLYG4')
...
Dont_match <- test[which(test[,'NEON_Exotic_Status'] != test[,'USDA_Exotic_Status']), ]
...
#Reducing all rows of the no match file to just the genus and specific epithet (no variety of subspecies) so that USDA can find matches
#no_match$bestname <- stringr::word(no_match$bestname, 1, 2)

#no_match$bestname  <- gsub('[[:punct:]]', "", no_match$bestname)
...
VegBank_AK3 <- left_join(VegBank_with_SpCode_AK, NEON_exotic_status, by = 'SpCode')
...
VegBank_AK5 <- VegBank_AK5  %>%
  select(-'VegBank_GrowthForm', -'VegBank_ExoticStatus')
