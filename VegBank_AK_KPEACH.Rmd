...
VegBank_AK <- full_join(VegBank_plot_data_AK2, VegBank_plot_species_list_AK2, by = 'UniqueID')   # JB - Full join might create duplicate. It is generally better to set up things to use left_join
...
USDA_plant_list_ALL <- read.delim2(file.path(shared_data_dir, "raw_VegBank_data/USDA_all_with_syn.txt"),  sep = ",")

USDA_list_w_exotic_status <- read.delim2(file.path(shared_data_dir, "USDA_list_w_exotic_status.txt"),  sep = ",")
...
#Save as csv for Jeff to inspect
# write.csv(no_match, file.path(data_output, "no_match_AK_KPEACH.csv"), row.names = FALSE)
...
#As csv
write.csv(VegBank_with_SpCode_AK, file.path(data_output, "VegBank_AK_KPEACH.csv"), row.names = FALSE)
