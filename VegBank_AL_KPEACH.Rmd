...
VegBank_plot_data_AL <- read_excel("/home/shares/neon-inv/raw_VegBank_data/CVS-ALplots.xlsx", sheet = "plot data")
VegBank_plot_species_list_AL <- read_excel("/home/shares/neon-inv/raw_VegBank_data/CVS-ALplots.xlsx",  sheet = "plot species list")
...
VegBank_AL <- full_join(VegBank_plot_data_AL2, VegBank_plot_species_list_AL2, by = 'UniqueID')
...
VegBank_AL <- VegBank_AL %>% add_column(Dataset = 'VEGBANK', .before = "UniqueID")  %>% add_column(Zone = 'L48', .before = "UniqueID")
...
#182 rows with no match
no_match <- dplyr::anti_join(VegBank_AL,USDA_plant_list_ALL, by="bestname")
...
#4,274
VegBank_with_SpCode_AL1 <- dplyr::inner_join(VegBank_AL, USDA_plant_list_ALL, by="bestname")

VegBank_with_SpCode_AL2 <- dplyr::inner_join(no_match, USDA_plant_list_ALL, by="bestname")

VegBank_with_SpCode_AL <- merge(VegBank_with_SpCode_AL1, VegBank_with_SpCode_AL2, all = TRUE)
...
#0! Yay!
no_match2 <- dplyr::anti_join(VegBank_with_SpCode_AL,USDA_plant_list_ALL, by="bestname")
...
plot_sizes <- unique(VegBank_with_SpCode_AL$`Taxon Observation Area`)

odd_plot_sizes <- VegBank_with_SpCode_AL %>%
  filter(`Taxon Observation Area` == '-1')

#write.csv(odd_plot_sizes,"odd_plot_sizes_AL_KPEACH.csv", row.names = FALSE)
...
#As csv
write.csv(VegBank_with_SpCode_AL,"/home/shares/neon-inv/output_files/VegBank_AL_KPEACH.csv", row.names = FALSE)
