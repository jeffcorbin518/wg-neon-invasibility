...
#Import VegBank Data from shared folder
...
VegBank_AL <- VegBank_AL %>% add_column(Dataset = 'VEGBANK', .before = "VegBankUniqueID")  %>% add_column(Zone = 'L48', .before = "VegBankUniqueID")
...
class(VegBank_with_SpCode_AL$`Taxon Observation Area`)
...
VegBank_with_SpCode_AL$`Taxon Observation Area` <- as.numeric(VegBank_with_SpCode_AL$`Taxon Observation Area`)

odd_plot_sizes <- VegBank_with_SpCode_AL  %>%
  filter(`Taxon Observation Area` < 0)

#VegBank_AL5 <- VegBank_with_SpCode_AL %>%
#  filter(`Taxon Observation Area` > 0)

write.csv(odd_plot_sizes,"/home/shares/neon-inv/output_files/odd_plot_sizes_AL_KPEACH.csv", row.names = FALSE)
