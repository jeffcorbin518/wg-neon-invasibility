...
VegBank_plot_data_AL <- read_excel(file.path(data_raw, "CVS-ALplots.xlsx"), sheet = "plot data")
VegBank_plot_species_list_AL <- read_excel(file.path(data_raw, "CVS-ALplots.xlsx"),  sheet = "plot species list")
...
duplicates <- VegBank_AL6 %>% group_by(VegBankUniqueID, SpCode) %>% summarise(n_obs = n()) %>% filter(n_obs > 1)

VegBank_AL6 %>% distinct()
