...
taxonomy_temp10_revised <- taxonomy_temp10_revised %>% select('Symbol', 'inv_L48', 'GrowthForm', 'bestname') %>% rename('SpCode' = 'Symbol', 'NEON_GrowthForm' = 'GrowthForm')

multStatusSpL48 <- multStatusSpL48 %>% select('SpCode', 'FINAL DECISION (L48)') %>% rename('Exception_Exotic_Status' = 'FINAL DECISION (L48)')

exotic_status1 <- dplyr::full_join(taxonomy_temp10_revised, multStatusSpL48, by = 'SpCode')
...
KPEACH_ExoticStatus_authority1 <- exotic_status2 %>% select('SpCode', 'NEW_ExoticStatus', 'NEON_GrowthForm', 'bestname')
...
KPEACH_ExoticStatus_authority <- KPEACH_ExoticStatus_authority3 %>% select(-'SpCode.y') %>% rename('SpCode' = 'SpCode.x')
